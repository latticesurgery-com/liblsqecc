...
std::unique_ptr<Layout> make_edpc_layout(size_t num_core_qubits, size_t num_lanes, const DistillationOptions& distillation_options)
{
    ...
    for (size_t i = t_distillation_region_rows; i<grid.size()-t_distillation_region_rows; i++) 
    {
        for (size_t j = t_distillation_region_cols; j<grid[i].size()-t_distillation_region_cols; j++)
        {
            ...
            if ((i-t_distillation_region_rows)%(num_lanes+1)==0 && (j-t_distillation_region_cols)%(num_lanes+1)==0) 
            {
